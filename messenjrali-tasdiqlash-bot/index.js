...
const BACKEND_URL = process.env.BACKEND_URL || 'https://backend-production-37a60.up.railway.app';
...
function sendControlMessage(chatId, text) {
...
app.post('/internal/send-reset-code', async (req, res) => {
  try {
    ...
    await sendControlMessage(
      chatId,
      `ExpertLine parolni tiklash kodi: ${code}\n\n` +
        'Bu kodni hech kimga bermang. Saytdagi parolni tiklash formasiga kiriting.'
    );
...
app.post('/internal/send-registration-code', async (req, res) => {
  try {
    const controlHeader = req.headers['x-bot-control-token'];
    if (!BOT_CONTROL_TOKEN || controlHeader !== BOT_CONTROL_TOKEN) {
      return res.status(403).json({ message: 'Forbidden' });
    ...
    await sendControlMessage(
      chatId,
        `ExpertLine ro'yxatdan o'tish: telefon tasdiqlash kodi: ${code}\n\n` +
        'Bu kodni hech kimga bermang. Ro\'yxatdan o\'tish sahifasidagi maydonga kiriting.'
    );

    return res.json({ success: true });
  } catch (err) {
    console.error('Failed to send registration code via bot:', err?.message || err);
