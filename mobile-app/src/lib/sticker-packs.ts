/** Google Noto Emoji — chat uchun PNG (tez, ishonchli); animated webp og‘ir. */
...
/** `U+1F600` → `1f600` */
export function unicodeCodeToNotoHex(code: string): string {
    return String(code || '')
        .trim()
        .replace(/^U\+/i, '')
        .toLowerCase();
...
export function notoStickerUrl(code: string, size: 128 | 512 = 512): string {
    const hex = unicodeCodeToNotoHex(code);
    if (!hex) return '';
    // 512.png ~30KB; 512.webp animated ~300–600KB (chatda spinner bilan qolib ketardi)
    return `${NOTO_CDN}/${hex}/${size}.png`;
}

/**
 * Eski telemoji / noto webp URL larni ishlaydigan PNG manbaiga aylantiradi.
 */
export function resolveStickerMediaUrl(raw: string | null | undefined, code?: string | null): string {
    if (code) {
        const fromCode = notoStickerUrl(code, 512);
        if (fromCode) return fromCode;
    }
    const url = String(raw || '').trim();
    if (!url) return '';

    const telemoji = url.match(/U\+([0-9A-Fa-f]+)(?:_\d+)?\.(?:webp|png)/i);
    if (telemoji) return notoStickerUrl(`U+${telemoji[1]}`, 512);

    const notoHex = url.match(/notoemoji\/latest\/([0-9a-f]{2,8}(?:_[0-9a-f]{2,8})*)\//i);
    if (notoHex) return notoStickerUrl(notoHex[1], 512);

    if (url.includes('telemoji')) {
        const m2 = url.match(/U\+([0-9A-Fa-f]+)/i);
        if (m2) return notoStickerUrl(`U+${m2[1]}`, 512);
    }

    if (/^https?:\/\//i.test(url) && /\.(webp|png)(\?|#|$)/i.test(url)) {
        return url.replace(/\/512\.webp(\?|#|$)/i, '/512.png$1');
    }
    return url;
}

function s(emoji: string, code: string): Sticker {
    return {
        emoji,
        code,
        webp: notoStickerUrl(code, 512),
        lottie: '',
    };
}
